...
namespace ApiPedidos.Tests
{
    public class PedidosTest
    {
        ...
        public async Task Test_Post_Pedidos_Returns_Created()
        {
            ...
            var httpClient = new WebApplicationFactory<Program>()
            .CreateClient();
    ...
    public class PedidoFaker : Faker<PedidoCreateCommand>
    {
        public PedidoFaker()
        {
            ...
            RuleFor(p => p.ProtocoloPagamento, f=> f.Random.Guid());
            RuleFor(p => p.DetalhesPagamento, f => f.Finance.AccountName());
            ...
            RuleFor(p => p.Itens, f =>
            {
                ...
                .RuleFor(i => i.Quantidade, f => f.Random.Int(1, 1000));
                return itensFaker.Generate(3);
            });
            FinishWith((f, pedido) =>
            {
                var totalPedido = pedido.Itens.Sum
